...
{
public:
  Grid_layout (int _rows, int _cols)
      : rows {_rows}
      , cols {_cols}
  {
  }

  void insert (AGUI::Widget& widget, int row, int col)
  {
    static float prev_width = 0;
    prev_width              = widget.size().y;

    assert (row <= rows && col <= cols);

    widget.move ({col * prev_width + padding, row * (widget.size().y + padding)});
  };
...
void AGUI::Frame::draw (void)
{
  ...
  minimize_button->click_event ([&]()
                                {
    if (minimized) {
      border.set_size_h (content.get_height() + frame_bar.get_height() +
                         style.bd_width.value());
      minimized = false;
    } else {
...
void AGUI::Frame::resize (void)
{
  ...
  if (resizing)
  {
    ...
    float clamp_x  = std::clamp (io.mouse_pos.x - resize_offset.x, min,
                                 std::numeric_limits<float>::infinity());
    float clamp_y =
        std::clamp (io.mouse_pos.y - resize_offset.y,
                    frame_bar.get_height() + resize_box.get_height(),
                    std::numeric_limits<float>::infinity());

    frame_bar.set_size_w (clamp_x - bd_width);
    content.set_size ({clamp_x - bd_width, clamp_y - bd_width});
    border.set_size ({clamp_x, clamp_y});
    resize_box.set_pos ({frame_bar.get_width() - resize_box.get_width(),
