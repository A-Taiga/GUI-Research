...
// NOTE:
// not using float here because it make the texture look jagged for some reason
// need to make a proper font atlas
void AGUI::SDL2::Backend::draw_text (const Vec2& pos, std::string_view str, color_t color) const
{
    int currecnt_x = pos.x;
    for (const auto& c : str)
    {
        const auto& character = character_map[c];
        SDL_Rect    rect      = {currecnt_x, (int) pos.y, character.w, character.h};
        SDL_SetTextureColorMod (character.texture, TO_RGB (color));
        SDL_RenderCopy (renderer, character.texture, nullptr, &rect);
